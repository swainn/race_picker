...
A gamified random-selection tool with fourteen game modes — race cars, battle bots, light cycles, Plinko balls, wall climbers, a battleship grid, a spinning wheel, a kung-fu brawl, a retro Space Invaders assault (as the invaders or the defenders), a one-on-one Street Duel, an alien abduction, a hand of poker, or a claw machine — all sharing the same participant list.
...
- **🛸 Alien Abduction** — Everyone scatters across a moonlit field while a saucer prowls overhead with its beam dark, strafing side to side a few times before switching the tractor beam on at a random moment and starting the hunt. Whoever gets pulled into the ship wins; a hard enough gust of wind can shove someone out of the beam and drop them back to safety. When only one participant is left un-abducted, they are revealed as an alien who was in disguise the whole time. 11 abductee sub-modes — the default **farm animals** deals each participant a random cow, horse, sheep, pig or chicken, plus single-species options (humans, cows, horses, chickens, sheep, pigs, cats, dogs, robots) and an all-in **mixed** — a weather setting, and synth sound: a wavering tractor-beam hum, capture and escape stings, wind gusts, and an eerie night-sky soundtrack, each toggleable
- **🃏 Poker Night** — Everyone is dealt two hole cards at an oval table and a shared board runs out flop, turn and river; whoever holds the **weakest** hand at showdown busts out, and the last player never busted is champion. Seat tiles show each player's live hand as the board develops, with an **AT RISK** marker that moves between players — someone doomed on the turn can be rescued by the river. Ties for worst (common enough that they fire on roughly a third of hands at a full table) go to a **sudden-death draw** where the lowest card busts; when one decides a hand, the result dialog shows the drawn card and how many players were tied. A collapsible **hand-ranking chart** sits below the table (shown by default) listing all ten hands strongest to weakest with example cards. The final leaderboard ranks everyone by **pots won** (holding the best hand at a showdown), so it reads the same whichever rule is active. A **pick** setting flips the whole rule: leave it on *worst hand busts out*, or switch to *best hand takes the pot*, where the strongest hand is the pick and the first pot won places first — the markers, wording, tie-break direction and standings order all follow. This is the only mode whose pick is fair by construction: an honest shuffle distributes the weakest and strongest hand uniformly alike, so nothing needs to compensate for seating.
- **🕹 The Claw** — The participants are little three-eyed alien toys in an arcade claw machine. Each drop, the claw sweeps the tank a few times before committing, descends on one toy while the others raise their arms in reverence ("OOOOOOH…"), clamps, carries it to the prize chute and drops it out — that toy is the pick. The claw's target is drawn uniformly *before* it starts moving and the whole hunt is choreography, so where a toy sits in the tank has no bearing on whether it gets taken. Speed and sound settings, with a synth cabinet: coin drop, gantry motor, the reverent "oooooh" swell, prong clamp, chute rattle, and an arcade jingle.

All modes share the same participant list and saved groups. Modes with a 🎛 Settings button (Racing, Wall Climber, Battleship, Wheel, Kung Fu, Space Invaders, Space Defenders, Street Duel, Alien Abduction, Poker Night, The Claw) expose extra per-mode options in the ☰ header menu, which also holds the participant manager and a global sound mute. Switching modes mid-race prompts a confirmation and resets the current race.
