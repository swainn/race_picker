...
- **🃏 Poker Night** — Everyone is dealt two hole cards at an oval table and a shared board runs out flop, turn and river; whoever holds the **weakest** hand at showdown busts out, and the last player never busted is champion. Seat tiles show each player's live hand as the board develops, with an **AT RISK** marker that moves between players — someone doomed on the turn can be rescued by the river. Ties for worst (common enough that they fire on roughly a third of hands at a full table) go to a **sudden-death draw** where the lowest card busts. This is the only mode whose pick is fair by construction: an honest shuffle distributes the worst hand uniformly, so nothing needs to compensate for seating.
