...
const IDLE: AiIntent = { targetId: null, desiredVx: 0, desiredVy: 0, move: null, block: false };
...
export function decideIntent(ctx: AiContext): AiIntent {
  ...
  // (5) Reactive block: an opponent winding up a move while facing us.
  let block = false;
  for (const o of others) {
    const od = Math.hypot(o.x - self.x, o.y - self.y);
    const facingUs = Math.sign(self.x - o.x) === o.facing || od < 22;
    if (od < 44 && o.movePhase === 'windup' && facingUs) {
      block = true;
      break;
    }
  }
  ...
  return { targetId: target.id, desiredVx, desiredVy, move, block };
}
